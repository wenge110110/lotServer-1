...
function Install()
{
  ...
  mkdir -p "${AcceTmp}/bin/"
  mkdir -p "${AcceTmp}/etc/"
  wget --no-check-certificate -qO "${AcceTmp}/bin/${AcceBin}" "https://github.com/wenge110110/lotServer/raw/master/${MyKernel}"
  [ ! -f "${AcceTmp}/bin/${AcceBin}" ] && Uninstall "Download Error! Not Found ${AcceBin}. "
  Welcome;
  wget --no-check-certificate -qO "/tmp/lotServer.tar" "https://github.com/wenge110110/lotServer/raw/master/lotServer.tar"
  tar -xvf "/tmp/lotServer.tar" -C /tmp
  acce_ver=$(acce_check ${KNV})
  wget --no-check-certificate -qO "${AcceTmp}/etc/apx.lic" "https://github.com/wenge110110/jiasu/blob/main/keygen.php?ver=${acce_ver}&mac=${Mac}"
